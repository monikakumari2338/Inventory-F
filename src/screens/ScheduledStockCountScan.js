...
const ScheduledStockCountScan = ({route}) => {
  ...
  const handleIconClick = async () => {
    try {
      ...
      const data = products.creationProductsdto.map(item => {
      ...
      console.log('products newData: ', newData);
      //setProductData(responseData);
      navigation.navigate('CycleStart', {
        data: newData,
      });
    } catch (error) {
      console.log(error);
...
export default ScheduledStockCountScan;
